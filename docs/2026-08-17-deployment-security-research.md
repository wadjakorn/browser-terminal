...
## 4.5 `POST /api/image` — ผิวใหม่ที่เพิ่มเข้ามา (2026-08-25)

ปุ่มแนบรูปเพิ่ม endpoint ที่ **รับ body ขนาดใหญ่และเขียนไฟล์ลงดิสก์** ซึ่งเป็น
พฤติกรรมที่ endpoint อื่นในแอปนี้ไม่มี บันทึกไว้ให้ครบว่ากันอะไรไว้บ้าง

**สิ่งที่ *ไม่ใช่* ความเสี่ยงใหม่:** "ผู้ใช้ที่ล็อกอินแล้วเขียนไฟล์ลงโฮสต์ได้" —
แอปนี้ให้ shell ของเครื่องอยู่แล้ว การเขียนไฟล์จึงไม่ใช่การยกระดับสิทธิ์

**สิ่งที่กันไว้จริง:**

- `originAllowed()` แบบเข้ม ไม่ใช่ `loginOriginAllowed()` — endpoint นี้มีแต่เบราว์เซอร์
  เรียก จึงมี `Origin` เสมอ ไม่ต้องผ่อนให้ curl แบบที่ `/api/login` จำเป็นต้องทำ
  · ด่าน origin มาก่อนด่าน session โดยตั้งใจ
- `sessionValid()` — ไม่มี session = 401 ก่อนอ่าน body แม้แต่ไบต์เดียว
- เพดาน 16 MiB พร้อม `req.destroy()` ก่อน throw ตามวินัยเดียวกับ `readJsonBody`
  ไม่งั้นเป็นช่องกิน fd
- ชนิดของไฟล์มาจาก **magic bytes** เท่านั้น ไม่ใช่ `Content-Type` ที่ client ส่งมา
  นามสกุลที่เขียนลงดิสก์จึงเป็นค่าคงที่จาก whitelist ปลอมไม่ได้
- ชื่อไฟล์ประกอบจาก timestamp + `randomBytes` ทั้งหมด **ไม่มีส่วนใดมาจาก input
  ของผู้ใช้** จึงไม่มีทาง path traversal
- เพดานพื้นที่รวมของไดเรกทอรี ไม่ใช่แค่เพดานต่อไฟล์ — ยิงรัวๆ ไม่ทำให้ดิสก์เต็ม
- ปิดได้ด้วย `IMAGE_PASTE=0` แล้วตอบ 404 ไม่ใช่ 403 เพื่อไม่ประกาศว่ามี endpoint อยู่

**สิ่งที่ยอมรับไว้:** ไฟล์รูปอยู่บนดิสก์ได้นานถึง 24 ชั่วโมง (สิทธิ์ 0600 ในไดเรกทอรี
0700) การผูกอายุกับการปิด WebSocket จะทำให้รูปหายระหว่างที่ผู้ใช้แค่สลับแอปบนมือถือ
ซึ่งเป็นอาการที่แย่กว่ามาก
