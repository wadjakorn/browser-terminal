...
### ปุ่มขวาแบบลากและแบบล็อก

ตอนนี้ปุ่ม `RMB` ทำได้แค่ "แตะครั้งเดียว = คลิกขวาครั้งเดียว" ยังไม่มี:

- **ลากด้วยปุ่มขวา** — TUI บางตัวใช้ปุ่มขวาลากเลือกพื้นที่ ต้องให้ arm อยู่ข้าม
  `dragStart`/`dragMove`/`dragEnd` แทนที่จะถูก consume ทิ้งที่ `dragStart`
- **สถานะล็อก** (แตะสองครั้งค้างไว้แบบ sticky modifier) — สำหรับคนที่ต้องคลิกขวา
  ติดกันหลายครั้ง

ทั้งสองอย่างเพิ่มบน `web/pointer-arm.ts` ได้โดยไม่ต้องรื้อโครง เลื่อนไว้เพราะยังไม่มี
use case จริงที่เจอเอง — ปุ่มกลาง (`button: 1`) ก็เหมือนกัน
