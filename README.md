...
| ปุ่ม `⇄` ท้ายแถว | สลับชุดปุ่มระหว่างสองหน้า |

แถบปุ่มล่างแบ่งเป็นสองหน้า หน้าละ 6 ปุ่ม บวก `⇄` กับ `⌨` ที่ตรึงไว้ทุกหน้า
...
8 ช่องไม่ใช่ตัวเลขที่เลือกเอาสวย — จอแคบสุดที่รองรับคือ 360px ปุ่มขนาด 40×44px
บวกช่องไฟลงได้พอดี 8 ช่อง **ถ้าจะเพิ่มปุ่มต้องเพิ่มหน้า ไม่ใช่เพิ่มช่อง** ไม่งั้น
แถบจะกลับไปล้นจนต้องเลื่อนแนวนอน ซึ่งคือปัญหาที่การแบ่งหน้าเกิดมาเพื่อแก้
(`SLOTS_PER_ROW` ใน `web/keybar.ts` มีเทสคุมไว้)

`/` กับ `-` ไม่อยู่บนแถบเพราะอยู่หน้าแรกของแป้นสัญลักษณ์ Android กดสองชั้นก็ถึง
ต่างจาก `|` กับ `~` ที่ต้องกดสามชั้น
