...
export const BORDER_CHARS: ReadonlySet<string> = new Set([
  // เส้นตรงแนวตั้ง
  '│', '┃', '║', '▏', '▕', '┆', '┊', '╎', '╏', '▌', '▐', '|',
  // ตัวเชื่อมและมุม — อยู่ในคอลัมน์เดียวกับเส้นแบ่งตรงหัวท้ายกล่องและตรงเส้นคั่นแนวนอน
  //
  // ขาดชุดนี้ไปแล้วจะเจ็บเงียบๆ: แถวที่เส้นแบ่งกลายเป็น ┬ หรือ ┼ จะถูกนับเป็น "ไม่ผ่าน"
  // ทั้งที่มันคือเส้นแบ่งเดียวกัน จอที่เตี้ยและมีเส้นคั่นหลายเส้นจึงตกเกณฑ์ threshold
  // ได้ง่ายมาก แล้วผลที่ผู้ใช้เห็นคือ "บางทีก็ตัด sidebar ออกให้ บางทีก็ไม่"
  //
  // ปลอดภัยที่จะเพิ่ม เพราะอักขระวาดกล่องพวกนี้ไม่โผล่ในเนื้อความปกติ ต่างจาก '|'
  '┬', '┴', '┼', '├', '┤', '┳', '┻', '╋', '┣', '┫',
  '╦', '╩', '╬', '╠', '╣', '╤', '╧', '╪', '╥', '╨', '╫',
  '┌', '┐', '└', '┘', '┏', '┓', '┗', '┛', '╔', '╗', '╚', '╝', '╭', '╮', '╰', '╯',
]);
