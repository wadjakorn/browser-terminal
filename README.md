...
| ปุ่ม `⇄` ท้ายแถว | สลับชุดปุ่ม (โผล่เฉพาะตอนที่ปุ่มไม่ครบในแถวเดียว) |

แถบปุ่มล่าง**วัดความกว้างจริงแล้วจัดหน้าเอง** ไม่ได้ตั้งจำนวนปุ่มตายตัว จอกว้าง
เห็นปุ่มมากขึ้น จอแคบแบ่งหน้าเพิ่ม และจัดใหม่ทันทีตอนหมุนจอ (`ResizeObserver`)

| ความกว้าง | ผลลัพธ์ (วัดบนเบราว์เซอร์จริง) |
|---|---|
| 360px | 3 หน้า หน้าละ 5 ปุ่ม |
| 412px (Android ทั่วไป) | 2 หน้า หน้าละ 7 ปุ่ม |
| 740px+ (แนวนอน / แท็บเล็ต) | **หน้าเดียว ครบทุกปุ่ม ไม่มี `⇄` ให้กดเลย** |

ปุ่มทั้งหมดเรียงตามความถี่ที่ใช้จริง ตัวหน้าๆ จึงได้อยู่หน้าแรกเสมอไม่ว่าจอกว้างเท่าไร:

```
Esc  Tab  ⇧Tab  Ctrl  ↑  ↓  ←  →  Alt  ^C  |  ~  /  -
```

`⌨` ตรึงไว้ทุกหน้าเพราะเป็นทางเดียวที่เปิดคีย์บอร์ดได้ ถ้าไปซ่อนอยู่หน้าใดหน้าหนึ่ง
ผู้ใช้จะพิมพ์ไม่ออกจนกว่าจะเดาถูกว่าต้องกด `⇄` ก่อน

ตัวเลขที่คุมทุกอย่างคือ `MIN_BTN_PX` (40px) กับ `GAP_PX` ใน `web/keybar.ts`
**`GAP_PX` ต้องตรงกับ `gap` ของ `.keybar` ใน `style.css`** ไม่งั้นจำนวนช่องที่คำนวณ
จะเกินความจริงแล้วแถบจะล้น — มีเทสคุมว่าช่องที่นับได้กางออกแล้วต้องไม่เกินความกว้าง
ที่มี ตั้งแต่ 320px ถึง 1920px

สถานะ sticky ของ `Ctrl`/`Alt` อยู่ที่ `input-pipeline` ไม่ได้อยู่ที่ตัวปุ่ม จึงรอด
ทั้งการสลับหน้าและการหมุนจอ
